...
export async function checkProjectAccess(
  ...
  if (typeof window !== 'undefined') {
    ...
    const trialUser = localStorage.getItem('smart_language_current_session') || localStorage.getItem('edufinance.impersonate');
    if (trialUser) {
      return {
        hasAccess: true,
        status: 'AVALIAÇÃO',
...
export const APP_NAMES_MAP: Record<string, { name: string; url: string }> = {
  ...
  'all': { name: 'Ecossistema Montanha (5 Apps)', url: 'http://localhost:5173' }
};
